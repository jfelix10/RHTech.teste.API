...
namespace RHTech.Teste.API.Infra.Persistence.Repositories.Dapper.Empresas
{
    public class EmpresaRepository : IEmpresaRepository
    {
        ...
        public async Task<bool> InsertAsync(Empresa empresa)
        {
            var query = @"
                INSERT INTO Empresas (NomeEmpresa, CNPJ, CodTipoEmpresa, DataInclusao, DataAlteracao)
                VALUES (@NomeEmpresa, @CNPJ, @CodTipoEmpresa, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);";

            try
            {
                return await _dbConnection.ExecuteScalarAsync<bool>(query, empresa);
            }
            catch (Exception)
            {
                throw;
        ...
        public async Task<Empresa?> GetByIdAsync(Guid id)
        {
            ...
            return await _dbConnection.QueryFirstOrDefaultAsync<Empresa?>(query, new { Id = id });
        }
        ...
        public async Task<IEnumerable<Empresa>> GetAllAsync()
        {
            ...
            return await _dbConnection.QueryAsync<Empresa>(query);
        }

        public async Task<IEnumerable<Empresa>> GetFilteredAsync(string filter)
        {
            var query = @"
                SELECT * FROM Empresas
                WHERE LOWER(NomeEmpresa) LIKE LOWER(@Filter) OR CNPJ LIKE @Filter;";

            return await _dbConnection.QueryAsync<Empresa>(query, new { Filter = $"%{filter}%" });
        }
